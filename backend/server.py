...
UPLOADS_DIR = Path(
    os.environ.get("UPLOADS_DIR", str(Path(__file__).parent.parent / "uploads"))
)
UPLOADS_DIR.mkdir(exist_ok=True, parents=True)
(UPLOADS_DIR / "previews").mkdir(exist_ok=True, parents=True)
...
async def generate_previews(
    animation_data: Dict[str, Any], unique_filename: str
) -> Tuple[str, str]:
    """Generate preview image (and optionally video) for an animation.

    Returns a tuple of (preview_image_url, preview_video_url). The preview image
    will always be generated while the preview video is optional and may be an
    empty string if generation isn't supported.
    """

    previews_dir = UPLOADS_DIR / "previews"
    previews_dir.mkdir(parents=True, exist_ok=True)

    image_path = previews_dir / f"{unique_filename}.png"

    width = int(animation_data.get("w", 400)) or 400
    height = int(animation_data.get("h", 400)) or 400
    ...
    try:
        await asyncio.to_thread(_create_image)
    except Exception as e:
        logger.warning(f"Preview image generation failed: {e}")

    preview_image_url = f"/uploads/previews/{image_path.name}"
    preview_video_url = ""  # Video previews not implemented
...
async def get_current_subscription(
    ...
    return {
        ...
        "plan": subscription_service.get_plan(
...
async def upgrade_subscription(
    ...
    subscription = await subscription_service.create_subscription(current_user.id, tier)
    return subscription
...
async def create_payment_intent(
    ...
    if payment_data.payment_method == "stripe":
        result = await payment_service.create_stripe_payment_intent(
            current_user.id, payment_data.amount, payment_data.subscription_tier
        )
...
async def upload_template(
    ...
    try:
        if current_user is None:
            raise HTTPException(
                status_code=401, detail="Authentication required to upload templates"
            )
        # Validate file type
        filename_lower = file.filename.lower()
        if not (filename_lower.endswith(".json") or filename_lower.endswith(".lottie")):
            raise HTTPException(
                status_code=400, detail="Only .json and .lottie files are supported"
            )
        ...
        preview_url, preview_video_url = await generate_previews(
            animation_data, unique_filename
        )
...
async def create_export(
    ...
    if not await subscription_service.check_export_permissions(
...
async def bulk_import_upload(
    ...
    for file in files:
        try:
            file_url, asset_type, metadata = (
                await file_storage_manager.save_uploaded_file(file)
            )
            ...
            # Generate thumbnail
            thumbnail_url = await file_storage_manager.generate_thumbnail(
                file_url, asset_type
            )
...
async def bulk_import_create_templates(
    ...
    for item in items:
        try:
            ...
            errors.append(
                {"filename": item.get("filename", "unknown"), "error": str(e)}
            )
